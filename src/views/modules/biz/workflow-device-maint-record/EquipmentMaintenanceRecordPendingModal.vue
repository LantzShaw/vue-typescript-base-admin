        ...
        <a-col :span="12">
          <div style="display: flex; justify-content: space-between">
            <div class="pl-4 h-10 leading-10">
              <span class="font-bold">待处置传感器</span>
              <!-- <span class="font-bold text-green-500">{{ pendingSensorAmount }}个</span> -->
            </div>
            <div class="pr-4 h-10 leading-10">
              <a-button
                @click="handleBatchRemovePending"
                :disabled="pendingCheckedKeys.length === 0"
                type="danger"
                >批量移除</a-button
              >
            </div>
          ...
          <BasicTable :clickToRowSelect="false" @register="registerPendingTable">
...
<script lang="ts" setup>
  ...
  // 接口
  import {
    workflowDeviceMaintRecordHandlePending,
  ...
  // data
  import { sensorResultTableColumns } from './equipmentMaintenanceRecord.data';
  ...
  const pendingCheckedKeys = ref<Key[]>([]);
  const pendingSensorAmount = ref<number>(0);
   ...
   * 待处置传感器 - 表格构建
   */
  const [registerPendingTable, { reload: reloadPendingTable }] = useTable({
    title: '',
    api: workflowDeviceMaintSensorPage,
    beforeFetch: (parmas) => {
      return {
    ...
    afterFetch: (data) => {
      pendingSensorAmount.value = data.length ?? 0;
    },
    pagination: true,
    columns: sensorResultTableColumns,
    useSearchForm: false,
    ...
    rowSelection: {
      type: 'checkbox',
      selectedRowKeys: pendingCheckedKeys,
      onSelect: onPendingSelect,
      onSelectAll: onPendingSelectAll,
    },
    actionColumn: {
      width: 80,
      title: '操作',
  ...
  async function handleBatchPending() {
    ...
    await workflowDeviceMaintRecordHandlePending({
      id: props.id,
   ...
   * 待处置传感器列表 - 批量移除
   */
  async function handleBatchRemovePending() {
    const bizWorkflowDeviceMaintSensorList = pendingCheckedKeys.value.map((key) => {
      return {
        id: key,
      };
    });
   ...
   * @param selected
   */
  function onPendingSelect(record, selected) {
    if (selected) {
      pendingCheckedKeys.value = [...pendingCheckedKeys.value, record.id];
    } else {
      pendingCheckedKeys.value = pendingCheckedKeys.value.filter((id) => id !== record.id);
    }
  }
   ...
   * @param changeRows
   */
  function onPendingSelectAll(selected, selectedRows, changeRows) {
    const changeIds = changeRows.map((item) => item.id);
    if (selected) {
      pendingCheckedKeys.value = [...pendingCheckedKeys.value, ...changeIds];
    } else {
      pendingCheckedKeys.value = pendingCheckedKeys.value.filter((id) => {
        return !changeIds.includes(id);
      });
    }
  }
