...
<script lang="ts" setup>
  ...
  import {
    workflowDeviceMaintPlanRegionList,
    workflowDeviceMaintPlanRegionAdd,
  ...
  const [registerTable, { reload, getForm, getSelectRowKeys, clearSelectedRowKeys }] = useTable({
    title: '',
    api: workflowDeviceMaintPlanRegionList,
    rowSelection: {
      type: 'checkbox',
    },
    rowKey: 'id',
  ...
  async function handleBatchBind() {
    if (getSelectRowKeys().length > 0) {
      createConfirm({
        ...
        onOk: async () => {
          await workflowDeviceMaintPlanRegionAdd({
            planId: planId.value,
            organizationId: organizationId.value,
            regionIds: getSelectRowKeys(),
          });
