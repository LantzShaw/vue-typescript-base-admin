...
<script lang="ts" setup>
  ...
  const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
    ...
    if (unref(isUpdate)) {
      // 请求数据
      record.value = ((await associationDeviceForm({ planId: data?.record?.id })) ||
        {}) as Recordable;
  ...
  async function handleSubmit() {
    ...
    try {
      setModalProps({ loading: true, confirmLoading: true });
      if (unref(isUpdate)) {
        await associationDeviceUpdate({ ...formState, planId: idRef.value });
      } else {
        await associationDeviceAdd({ ...formState, planId: idRef.value });
      }
